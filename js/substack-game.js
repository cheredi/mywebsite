(function () {
  async function loadSubstackPosts(options = {}) {
    const { manual = false } = options;
    const feedUrl = 'https://lipgloss2llms.substack.com/feed';
    const heading = document.getElementById('substackHeading');
    const description = document.getElementById('substackDescription');
    const status = document.getElementById('substackStatus');
    const container = document.getElementById('substackPosts');

    if (!heading || !description || !status || !container) {
      return;
    }

    if (manual) {
      status.textContent = 'Refreshing posts from RSS...';
    }
    ...
    const parseFeed = (xmlText) => {
      ...
      const channelTitle = xmlDoc.querySelector('channel > title')?.textContent?.trim() || 'Substack';
      const channelDesc = xmlDoc.querySelector('channel > description')?.textContent?.replace(/\s+/g, ' ').trim() || '';
      const items = [...xmlDoc.querySelectorAll('item')].slice(0, 4).map(item => {
        const title = item.querySelector('title')?.textContent?.trim() || 'Untitled Post';
        const link = item.querySelector('link')?.textContent?.trim() || 'https://lipgloss2llms.substack.com';
        const pubDateRaw = item.querySelector('pubDate')?.textContent?.trim() || '';
        const pubDate = pubDateRaw ? new Date(pubDateRaw).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) : 'Recent post';
        const excerptRaw = item.querySelector('description')?.textContent?.trim() || '';
        const excerpt = excerptRaw.replace(/\s+/g, ' ').slice(0, 170);
        return {
          title,
          link,
          pubDate,
      ...
      return {
        ...
        channelDesc,
        items,
        lastUpdated: xmlDoc.querySelector('lastBuildDate')?.textContent?.trim() || ''
      };
    };
    ...
    try {
      ...
      if (!response.ok) {
        response = await fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent(feedUrl)}`);
      }

      const xmlText = await response.text();
      const feedData = parseFeed(xmlText);

      heading.textContent = `${feedData.channelTitle} (Substack)`;
      description.textContent = feedData.channelDesc || 'Latest posts from my Substack.';
      status.textContent = feedData.lastUpdated ? `Auto-updated from RSS. Last updated: ${new Date(feedData.lastUpdated).toLocaleString()}` : 'Auto-updated from RSS.';

      container.innerHTML = '';
      feedData.items.forEach(post => {
        const article = document.createElement('article');
        article.className = 'writing-card tilt-card';
        article.innerHTML = `
          <h3>${post.title}</h3>
          <div class="meta">${post.pubDate}</div>
          <p>${post.excerpt}</p>
          <a href="${post.link}" target="_blank" rel="noopener noreferrer" class="btn btn-secondary">Read on Substack</a>
        `;
        container.appendChild(article);
      });
    } catch (error) {
      description.textContent = 'I write about applied AI, career pivots, and real-world tech lessons.';
      status.textContent = 'Could not fetch live posts right now. Use the button below to open Substack directly.';
      container.innerHTML = '';
    }
  }

  const refreshSubstackBtn = document.getElementById('refreshSubstackBtn');
  if (refreshSubstackBtn) {
    refreshSubstackBtn.addEventListener('click', () => {
      loadSubstackPosts({ manual: true });
    });
  }
